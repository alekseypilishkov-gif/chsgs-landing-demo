...
export function setDebugViewMode(mode: DebugViewMode): void {
  const input = document.querySelector<HTMLInputElement>(`#debug-panel input[data-debug="viewMode"][value="${mode}"]`);
  if (!input) return;
  input.checked = true;
...
type DebugTabId = 'model' | 'cup' | 'plate' | 'day_and_night';
const isTabId = (value: string | undefined): value is DebugTabId =>
  value === 'model' || value === 'cup' || value === 'plate' || value === 'day_and_night';
